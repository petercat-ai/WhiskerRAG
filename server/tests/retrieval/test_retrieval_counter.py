...
from server.core.retrieval_counter import RetrievalCounter
...
class TestRetrievalCounter(unittest.TestCase):
    ...
    def test_record(self):
        self.counter.record("test_key")
        shard_id = self.counter._get_shard("test_key")
        with self.counter.locks[shard_id]:
            self.assertEqual(self.counter.active_buffers[shard_id]["test_key"], 1)
        time.sleep(1.5)
        with self.counter.locks[shard_id]:
            self.assertEqual(self.counter.active_buffers[shard_id]["test_key"], 0)

        db_result = self.counter.db_plugin.retrieval_count
        self.assertDictEqual(db_result, {"test_key": 1})
