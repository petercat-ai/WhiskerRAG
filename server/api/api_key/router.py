...
async def create_api_key(
    ...
    logger.info("[create_api_key][start], req=%s", body)
    try:
        ...
        logger.error("[create_api_key][error], req=%s, error=%s", body, str(e))
...
async def update_api_key(
    ...
    logger.info("[update_api_key][start], req=%s", body)
    try:
        db_engine = PluginManager().dbPlugin
        ...
        logger.error("[update_api_key][error], req=%s, error=%s", body, str(e))
...
async def get_api_key_list(
    ...
    logger.info("[get_api_key_list][start], req=%s", body)
    try:
        db_engine = PluginManager().dbPlugin
        api_keys = await db_engine.get_tenant_api_keys(tenant.tenant_id, body)
        return ResponseModel(data=api_keys, success=True)
    except Exception as e:
        logger.error("[get_api_key_list][error], req=%s, error=%s", body, str(e))
        raise HTTPException(status_code=500, detail="Failed to get API key list")
...
async def change_api_key_status(
    ...
    logger.info("[deactivate_api_key][start], key_id=%s", body.key_id)
    try:
        db_engine = PluginManager().dbPlugin
        existing_key = await db_engine.get_api_key_by_id(tenant.tenant_id, body.key_id)
        if not existing_key or existing_key.tenant_id != tenant.tenant_id:
        ...
        logger.error(
            "[deactivate_api_key][error], key_id=%s, error=%s", body.key_id, str(e)
        )
        raise HTTPException(
            status_code=500, detail="Failed to change API key active status"
        )
...
async def get_all_expired_api_keys(
    ...
    logger.info("[get_all_expired_api_keys][start]")
    try:
        db_engine = PluginManager().dbPlugin
        ...
        logger.error("[get_expired_api_keys][error], error=%s", str(e))
        raise HTTPException(status_code=500, detail="Failed to get expired API keys")
